...
def get_options(correct_word, all_words):
    ...
    options = random.sample(all_words, 4)
...
def get_time_remaining():
    ...
    return max(0, 30 - elapsed_time)  # 30 saniyelik süre
...
st.title("🎉 Eğlenceli Kelime Tahmin Oyunu 🎉")
st.subheader(f"Skor: {st.session_state.score}")
st.write(f"Zaman: {int(get_time_remaining())} saniye kaldı!")

# Eğer zaman dolduysa
if get_time_remaining() == 0:
    st.error("Zaman doldu! Yeni bir kelimeye geçiliyor...")
    time.sleep(2)
...
if st.button("Cevabı Kontrol Et"):
    ...
    if user_choice == kelime:
        st.success(f"✔️ Tebrikler! Doğru cevap: {kelime}")
        st.session_state.score += 1
        st.balloons()  # Doğru cevabı kutlamak için balon animasyonu
    else:
        st.error(f"❌ Yanlış cevap! Doğru cevap: {kelime}")
        st.session_state.score -= 1
    time.sleep(2)  # Cevap gösteriminden sonra 2 saniye bekleyip yeni soruya geç
    st.experimental_rerun()

# Yeni kelimeye geçiş butonu
